...
module ActiveShipping
  class UPS < Carrier
    ...
    def find_all_tracking_numbers(shipment_number, options={})
      ...
      parse_ship_tracking_response(response, options)
    end
      ...
      if success
        ...
        unless status == :delivered
          scheduled_delivery_date = parse_ups_datetime(
            :date => (first_shipment.get_text('ScheduledDeliveryDate') || first_package.get_text('RescheduledDeliveryDate')),
            :time => nil
            )
        end
    ...
    def parse_ship_tracking_response(response, options = {})
      xml = REXML::Document.new(response)
      success = response_success?(xml)
